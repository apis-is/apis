...
function parseJavaScriptVariable(body) {
  ...
  var res_string = jsonString.replace(/(:\'[-0-9][0-9]*)(,)([0-9]*)/g, '$1.$3');

  // Create a Regular expression and change date representation.
  var regex_date = /(\'t\':)new Date\((([0-9.,-]+),([0-9.,-]+),([0-9.,-]+),([0-9.,-]+),([0-9.,-]+),([0-9.,-]+))\)(,\'a\')/g;
  var dateReplace = function (match, p1, p2, p3, p4, p5, p6, p7, p8, p9, offset, stringg) {
    var parsed_date = new Date(parseInt(p3), parseInt(p4.split("-")[0]) - 1, parseInt(p5), parseInt(p6), parseInt(p7), parseInt(p8));
    return p1 + '\'' + parsed_date.toISOString() + '\'' + p9;
  }

  try {
    // Create semi-final JSON string.
    var res = JSON.parse(res_string.replace(regex_date, dateReplace).replace(/\'/g, '"'));
  } catch(ex) {
