...
describe('bus', function() {
  ...
  var customCheck = function(json) {
    ...
    helpers.assertPresenceOfFields(
      ['unixTime', 'x', 'y', 'from', 'to'],
      busses
    );
  };
  ...
  describe('realtime', function() {
    ...
    //   it('should return an array of objects containing correct fields', function(
    //     done
    //   ) {
    ...
    describe('when the data source returns an error', () => {
      before(function() {
        sinon
          .stub(request, 'get')
          .yields(
            "We don't want no scrapers around these here parts",
            null,
            null
          );
      });
      ...
      it('should return a appropriate message', done => {
        getBusRoutes({})
          .then(data => {
            should.fail(
              'Got back data even though the data source returned an error'
            );
          })
          .catch(error => done());
      });
