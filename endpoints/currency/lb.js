...
app.get('/currency/lb', function (req, res) {
    ...
    var type = req.params.type || 'A'; // type is not a reserved keyword, typeof is
    ...
    request.get({
        ...
        if (err || response.statusCode !== 200) {
            return res.json(500, {
                error: 'www.landsbankinn.is refuses to respond or give back data'
            });
        }
        ...
        parseString(xml, {
            ...
            for (var i = 0, length = arr.length, currency = arr[i]; i < length; i++) {
                currencies.push({
